...
export default function RoomPage () {
    ...
    useEffect(() => {
        const admin = Boolean(user && room && String(user.id) === String(room.primaryAdminId));
        setIsAdmin(admin);
        if (process.env.NODE_ENV === 'development') console.log('[Room] admin check', { userId: user?.id, primaryAdminId: room?.primaryAdminId, isAdmin: admin });
    }, [ user, room ]);
